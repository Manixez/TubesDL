...
class ResNet34Model(nn.Module):
    """
    ResNet-34 model untuk face recognition
    Menggunakan pretrained ResNet-34 dari torchvision
    """
    def __init__(self, num_classes, pretrained=True, device=None):
        ...
        # ResNet-34 original FC: nn.Linear(512, 1000)
        ...
        # ResNet-34 menghasilkan embedding 512-dimensional
    ...
    def freeze_backbone(self):
        """Freeze ResNet-34 backbone untuk fine-tuning hanya classifier"""
        for param in self.resnet.parameters():
            param.requires_grad = False
    
    def unfreeze_backbone(self):
        """Unfreeze ResNet-34 backbone untuk full fine-tuning"""
        for param in self.resnet.parameters():
...
if __name__ == "__main__":
    ...
    num_classes = 70
    print(f"\nCreating ResNet-34 model for {num_classes} classes...")
    model = ResNet34Model(num_classes=num_classes, pretrained=True, device=device)
    
    print("\nModel components:")
    print(f"1. ResNet-34 (Backbone): {type(model.resnet).__name__}")
